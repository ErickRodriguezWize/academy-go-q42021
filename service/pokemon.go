...
func (sp PokemonService) GetPokemonWorker(pokemons []model.Pokemon, t string, items int, itemsPerWorker int) ([]model.Pokemon, error) {
	var wg sync.WaitGroup
	retrievePokemons := []model.Pokemon{}

	// Workers. Distribued items needed between workers.
	...
	// Sleep the main thread to sync.
	time.Sleep(time.Second)
	log.Printf("Results #: %v\n", len(retrievePokemons))

	// Switch-case to evaluate results from worker pool before sending the results.
	switch {
	case len(retrievePokemons) == 0:
		return retrievePokemons, pokerr.ErrNoPokemonResponse
	case len(retrievePokemons) > items:
		return retrievePokemons, pokerr.ErrNotMeetCriteria
	default:
		return retrievePokemons, nil
	}
